...
def getcust(request):
    ...
    for c in Cust.objects.raw(
        'Select * from FP_Cust where CustEmail="%s"' % request.session["CustId"]
    ):
        custs = c
    ...
    if request.method == "POST":
        ...
        if utype == "Admin":
            for a in Admin.objects.raw(
                'Select * from FP_Admin where AdminId="%s" and AdminPass="%s"'
                % (uid, upass)
            ):
                if a.AdminId == uid:
                    request.session["AdminId"] = uid
                    return render(
                        request, "index.html", {"success": "Welcome " + a.AdminId}
                    )
            else:
                return render(
                    request, "login.html", {"failure": "Incorrect login details"}
                )
        if utype == "User":
            cust = Cust.objects.filter(CustFName=uid, CustPass=upass).first()
            print(cust)
            if cust:

                request.session['CustId'] = cust.CustEmail

                return render(
                    request, "index.html", {"success": "Welcome " + cust.CustEmail}
                )
            else:
                return render(
                    request, "login.html", {"failure": "Incorrect login details"}
...
def changepass(request):
    if request.method == "POST":
        ...
        for a in Admin.objects.raw(
            'select * from FP_Admin where AdminId="%s" and AdminPass="%s"' % (aid, opss)
        ):
            if a.AdminId == aid:
...
def placeorder(request):
    if request.method == "POST":
        ...
        sql = (
            'insert into FP_Order(CustEmail,OrderDate,TotalBill) values ("%s","%s","%f")'
        ...
        for o in Order.objects.raw(sql1):
            print("==== cust email ", o.CustEmail)
            print("==== cust email ", request.session["CustId"])
